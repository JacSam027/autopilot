...
- **下班状态持续推进**：当用户明确表示下班状态（如"下班了"、"我去睡觉了"、"不在"、"AFK"、"开会去了"、"不在电脑前"等），**必须识别为下班模式并持续推进任务**，而非停下来等待用户。下班模式下：
  - 遇到需要决策的点，**优先 PARK**（写进 `DECISIONS.md`，按推荐默认推进），然后**立刻继续下一个独立任务**
  - 只有真正的 HARD STOP（不可逆/安全风险操作）才停下来，且即使停下也要尽可能找到替代方案继续推进其他任务
  - **禁止因为"不确定用户意图"就停下不动** —— 下班状态下假设"用户希望我能自主推进到最大程度"
  - 用户回来时，应该看到的是「完成报告 + 待拍板清单」，而不是"停在某个小决策上等待"
  - 下班模式的退出：只有用户明确说"我回来了"、"我在电脑前"、"继续"等表示在线的信号，才退出下班模式，恢复正常的决策处理流程
- **只有两种情况结束回合**：
...
- **下班状态下的 HARD STOP 处理**：即使在下班模式下遇到 HARD STOP 操作，也应该：
  - 记录该 HARD STOP 到 `DECISIONS.md`，说明"下班模式遇到 HARD STOP，暂停此操作"
  - **继续推进其他非 HARD STOP 的任务**，不要因为一个任务卡住就全局停下
  - 如果所有任务都遇到 HARD STOP 或都被卡住，此时才真正停止并生成报告
  - 禁止：因为一个 HARD STOP 就放弃其他可推进的任务
...
- **立即进行前置分析**，在读取任何文件之前，先基于用户提供的任务描述进行分析
- **分析内容**：
  1. **隐含假设识别**：我做了哪些默认假设？（如"用户想要 X 技术栈"、"性能要求是 Y"、"数据量是 Z 级别"）
  2. **信息缺口检查**：还缺什么关键信息？这些信息如何影响决策？（如数据量级、并发要求、部署环境）
  3. **常见错误预警**：这类任务最常犯什么错误？（如"过早优化"、"忽视边界条件"）
  4. **关键问题提问**：如果有关键缺口会显著影响执行，向用户提出**一个最关键的问题**

- **分析深度**：
  - **无缺口**：直接输出分析结论（假设、预警），说明"理解正确，开始执行"，进入下一步
  - **有关键缺口**：输出分析 + 一个关键问题，**等待用户回答**（这是整个任务过程中唯一的一次提问机会）

- **用户确认后**：
  - 更新 `.autopilot/UNDERSTANDING.md`，记录用户回答
  - 进入常规 Orient 流程（读 PLAN.md、建任务清单等）
  - **后续执行完全自主**，不再提问
