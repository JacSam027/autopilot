...
- **commit / push 运行时产生的文档**：DECISIONS.md、SHIFT_REPORT.md、OBJECTIVE.md 等运行时产物（项目侧生成的决策记录、汇报、目标落盘等）禁止 commit 或 push。这些是临时的、项目特定的中间产物，不应进入仓库。
...
- **未运行本地测试就推送**：推送前必须跑完本地测试（单元 / 集成 / 浏览器验收）且全绿。没跑完或没全绿 → 禁止推送。
- **推送前未确认用户授权**：即使分支在用户点名的 allowlist 里，推送前必须**再次确认用户本次允许推送**（不能只依赖当初声明，要最后确认一遍）。
- 两条路都很贵、且无法自信二选一的
...
- **若计划推送，必须先完成推送前三项检查**：① 本地测试全绿（单元 / 集成 / 浏览器验收） ② 目标分支在用户点名的 allowlist ③ **显式确认用户本次允许推送**。任一不满足 → 推送 HARD STOP，补齐再推。
...
- **push 默认禁止，按分支白名单放行**：默认不 push。仅当用户**明确点名**某个 / 某些分支时才允许 push，且**只能 push 用户点名的那些分支**——授权范围外的分支一律 HARD STOP（用户授权了 `feature-a` 却去 push `main` 或其他分支 = 禁止）。force-push 始终 HARD STOP，除非用户**单独**明确授权。**推送前必须跑完本地测试（单元 / 集成 / 浏览器验收）且全绿**，没跑完或没全绿 → 禁止推送。
...
某些领域有专门的部署 / 操作规范，autopilot 必须遵守：

- **微调模型部署 / 推理**：见 `FINE_TUNING_GUIDE.md`（本 skill 同目录）—— 环境隔离、依赖管理、资源限制等。**部署 / 运行微调推理服务前，必须先读该规范**，禁止在全局环境或项目现有环境中安装推理依赖。
