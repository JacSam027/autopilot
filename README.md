...
- **非破坏性写入** —— 对不可再生 / 昂贵再生的产物（实验数据、模型、快照、dump 等）默认**只新增不覆盖**，避免在一次 save 里静默销毁旧版本
- **上下文卫生** —— 长任务里主动给主上下文减负：笨重的读 / 分析派给只读子智能体只回收结论、定位式读取不整读、状态外化到文件、压缩后从落盘重建状态
- **目标保持** —— 长链任务把原始目标落盘到 OBJECTIVE.md，关键检查点回看防止迷路，压缩恢复时最先重建目标信息
...
### 非破坏性写入
对**再生成本高或无法复现**的产物（实验数据、训练好的模型 / checkpoint、历史快照、导出 dump、生成的大文件如索引 / embedding / 缓存），默认**只新增、不覆盖**——写新版本就开新目录 / 带版本号或时间戳的副本，旧的原地保留。覆盖前先判断能否廉价、确定性再生；拿不准就当不可再生。必须覆盖且确认旧内容无用才覆盖，否则记进 DECISIONS 按"新建保留旧版"推进。这把 DB 的"禁止 DELETE"延伸到了文件 / 目录层面的覆盖。

### 上下文卫生
长 shift 里主智能体的上下文会越攒越满、质量下降，**主动减负**和落盘同等重要。四个抓手：① **派子智能体当减压阀**——要读一大堆文件 / 翻长日志 / 跑大块分析时，派给只读子智能体（如 Explore），只拿回一段结论，不把原文灌进主上下文（"这步自己干会不会让 context 暴涨？"会就派）；② **定位式读取**——Grep / Glob 先定位再针对性 Read，要结论不要原文；③ **状态外化到文件**——进度走 TaskList、决策走 `DECISIONS.md`、过程走 `SHIFT_REPORT.md`，别装脑子里；④ **压缩恢复协议**——session 压缩 / 重启后先读 TaskList → OBJECTIVE → DECISIONS → SHIFT_REPORT 重建状态再继续。子智能体因此是三用途：并行推进、隔离 review、上下文减压。

### 目标保持
长链任务容易在执行中逐渐偏离原始目标——上下文膨胀、压缩、分岔都会导致"我还记得当初要做的吗"模糊。**把原始目标落盘并定期回看**是防止迷路的定锚。Orient 阶段写 `.autopilot/OBJECTIVE.md`（含原始目标陈述、成功/验收标准、约束/边界、不变项），每个阶段切换前、压缩后恢复时、做涉及范围/边界的决策前，先回看 OBJECTIVE.md 确认"还在做当初要做的"。只有用户明确提供新需求时才改 OBJECTIVE.md，且留痕（时间、旧目标、新目标、来源）；模型自作主张的理解改变不得改文件，只进 DECISIONS。

### 微调模型部署
部署 / 运行微调模型推理服务时，必须遵守环境隔离，禁止污染全局环境。规范详见 `FINE_TUNING_GUIDE.md`（与 SKILL.md 同目录），核心要求：**Docker 或新建 conda 环境运行推理服务，禁止在全局或项目现有环境中安装推理依赖**。部署前必须先读该规范。
