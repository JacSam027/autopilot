...
### 上下文卫生
长 shift 里主智能体的上下文会越攒越满、质量下降，**主动减负**和落盘同等重要。四个抓手：① **派子智能体当减压阀**——要读一大堆文件 / 翻长日志 / 跑大块分析时，派给只读子智能体（如 Explore），只拿回一段结论，不把原文灌进主上下文（"这步自己干会不会让 context 暴涨？"会就派）；② **定位式读取**——Grep / Glob 先定位再针对性 Read，要结论不要原文；③ **状态外化到文件**——进度走 TaskList、决策走 `DECISIONS.md`、过程走 `SHIFT_REPORT.md`，别装脑子里；④ **压缩恢复协议**——session 压缩 / 重启后先读 TaskList → DECISIONS → SHIFT_REPORT 重建状态再继续。子智能体因此是三用途：并行推进、隔离 review、上下文减压。
